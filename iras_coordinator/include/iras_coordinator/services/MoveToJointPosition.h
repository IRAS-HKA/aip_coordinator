...
class MoveToJointPosition : public RosService<MoveToJointPositionSrv>
{
public:
    static BT::PortsList providedPorts();

    MoveToJointPosition(const std::string &name, const BT::NodeConfiguration &config) : RosService(name, config) {}

    std::string ros2_service_name() override;

    void on_send(std::shared_ptr<MoveToJointPositionSrv::Request> request) override;
    bool on_result(std::shared_future<std::shared_ptr<MoveToJointPositionSrv::Response>> response, std::shared_ptr<MoveToJointPositionSrv::Request> request) override;
};