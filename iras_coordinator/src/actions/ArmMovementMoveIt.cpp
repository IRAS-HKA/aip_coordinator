...
void ArmMovementMoveIt::on_send(MoveArmMoveIt::Goal &goal)
{
    ...
    goal.pose.pose.position.z = ports.get_port_value<float>("z");

    tf2::Quaternion q;
    q.setRPY(DEG2RAD(ports.get_port_value<float>("rotation_x")), DEG2RAD(ports.get_port_value<float>("rotation_y")), DEG2RAD(ports.get_port_value<float>("rotation_z")));
    q.normalize();
    goal.pose.pose.orientation = tf2::toMsg(q);

    goal.cart = ports.get_port_value<bool>("cartesian");
    goal.speed = ports.get_port_value<float>("speed");
    ...
    log("Goal: (x=" + Converter::ftos(goal.pose.pose.position.x) +
        ...
        ")\n(rx=" + Converter::ftos(ports.get_port_value<float>("rotation_x")) +
        ", ry=" + Converter::ftos(ports.get_port_value<float>("rotation_y")) +
        ", rz=" + Converter::ftos(ports.get_port_value<float>("rotation_z")) + ")");
}
